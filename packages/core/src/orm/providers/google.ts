...
export interface GoogleUserPerson {
  ...
  birthdays: { date: { year: number; month: number; day: number }; metadata: { primary: boolean } }[];
}
...
export class GoogleOAuth extends OAuth2Client {
  ...
  public async callbackUser(code: string): Promise<GoogleUser> {
    ...
    const req = await this.request<GoogleUserInfo>({ url: 'https://www.googleapis.com/oauth2/v1/userinfo' });
